...
def county_decode(input, COUNTRY):
	...
	logging.debug(' '.join(['	P = >',P]))
	logging.debug(' '.join(['	SS = >',SS]))
	logging.debug(' '.join(['	CCC = >',CCC]))
	logging.debug(' '.join(['	SSCCC = >',SSCCC]))
	if COUNTRY=='US':
...
def same_decode(same, lang, same_watch=None, event_watch=None, text=True, call=None, command=None, jsonfile=None):
	...
	if msgidx != -1:
		...
		logging.debug(' '.join(['	Code found >',str(PSSCCC_list)]))
		US_bad_list=[]
		CA_bad_list=[]
		MX_bad_list=[]
		for code in PSSCCC_list:
			try:
				...
				logging.debug(' '.join(['	US KeyError Code found >',str(code)]))
			try:
				county=defs.CA_SAME_CODE[code[1:]]
			except KeyError:
				CA_bad_list.append(code)
				logging.debug(' '.join(['	CA KeyError Code found >',str(code)]))
			try:
...
#		if len(MX_bad_list) < len(CA_bad_list):
#			COUNTRY='MX'
...
#				COUNTRY='US'
		...
		logging.debug(' '.join(['	Invalid Code >',str(len(bad_list))]))
		logging.debug(' '.join(['	Country >',COUNTRY]))
		logging.debug('-' * 30)
		...
		if check_watch(same_watch, PSSCCC_list, event_watch, EEE):
			...
			if jsonfile:
				try:
					import json
					data=kwdict(ORG=ORG, EEE=EEE, TTTT=TTTT, JJJHHMM=JJJHHMM, STATION=STATION, TYPE=TYPE, LLLLLLLL=LLLLLLLL, COUNTRY=COUNTRY, LANG=defs.LANGUAGE, event=get_event(EEE), type=get_indicator(EEE), end=fn_dt(alert_end(JJJHHMM,TTTT)), start=fn_dt(alert_start(JJJHHMM)), organization=defs.SAME__ORG[ORG]['NAME'][COUNTRY], PSSCCC=PSSCCC, PSSCCC_list=PSSCCC_list, location=get_location(STATION, TYPE), date=fn_dt(datetime.datetime.now(),'%c'), length=get_length(TTTT), seconds=alert_length(TTTT), MESSAGE=MESSAGE)
